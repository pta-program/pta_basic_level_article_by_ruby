...
students.times do
  ...
  questions.times do |i|
    ...
    total += scores[i] if selected.sort == correct[i].sort
    total += scores[i] / 2 if selected.length == common && common.positive? && selected.sort != correct[i].sort
    (correct[i] | selected).each { |option| errors[i][option] += 1 if selected.include?(option) != correct[i].include?(option) }
  end
...
if maximum.zero?
  ...
  errors.each_with_index { |hash, i| hash.each { |option, value| puts "#{maximum} #{i + 1}-#{option}" if value == maximum } }
end
```
